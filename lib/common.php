...
function getUserImpactStats(PDO $pdo, $user_id) {
    $stmt = $pdo->prepare("
        SELECT 
            ...
            (SELECT COALESCE(SUM(helpful_votes), 0) FROM study_resources WHERE uploader_id = :uid) as helpful_votes,
            (SELECT COUNT(*) FROM suggestions WHERE author_id = :uid) as suggestions_made,
            (SELECT COUNT(*) FROM suggestions WHERE author_id = :uid AND status = 'implemented') as suggestions_implemented
    ");
...
/**
 * Register admin action on problemasHH (audit trail)
 * @param PDO $pdo Database connection
 * @param int $problema_id ID of the problema (from problemasHH table)
 * @param int $admin_id ID of admin performing action
 * @param string $tipo Action type: 'asignado', 'cambio_estado', 'nota_agregada', 'resuelto', 'reabierto'
...
function registrarAccionProblemaHH(PDO $pdo, $problema_id, $admin_id, $tipo, $detalles = '') {
    ...
    return $stmt->execute([
        ...
        ':admin_id' => $admin_id,
        ':tipo' => $tipo,
        ':detalles' => $detalles ?: 'se esta procesando'
    ]);
}
